        ...
        <div className="overflow-x-scroll md:overflow-x-auto">
          <ul className="grid grid-cols-1 gap-x-4 gap-y-6 lg:grid-cols-2 sm:gap-x-3 min-w-max  ">
            {camerasList.response && camerasList.response.data.camera.map((file) => (
              ...
              >
                {editCamera && <EditCamera setEditCamera={setEditCamera} currentId={ file.id}/>}
                ...
                <div>
                  <div>
             <span onClick={() => setEditCamera(true)}> <div className="text-primary text-xl ">
                  <AiTwotoneEdit /></div></span>
                    <div className="text-primary text-xl " onClick={() => handleDelete(file.id)}>
                     
                   
                <AiFillDelete color="red"/></div> <br/></div>
